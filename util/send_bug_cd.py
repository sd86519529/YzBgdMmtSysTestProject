...
def send_bug_to_cd(severity, module, title, pri, steps):
    import re
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.132 Safari/537.36'
    }
    res = requests.get(url='http://k3.hzyz.iotube.cn/zentao/user-login-L3plbnRhby9idWctYnJvd3NlLTEuaHRtbA==.html',
                       headers=headers)

    zentaosid = res.headers.get('Set-Cookie')
    a = re.findall('.*?=(.*?);', zentaosid)

    log_url = 'http://k3.hzyz.iotube.cn/zentao/user-login-L3plbnRhby9idWctYnJvd3NlLTEuaHRtbA==.html'
    log_headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.132 Safari/537.36',
    ...
    print(re.text)
    put_url = 'http://k3.hzyz.iotube.cn/zentao/bug-create-1-0-moduleID=0.html'
    put_headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.132 Safari/537.36",
...
def data_util(data, html):
    ...
    steps = '本次请求数据' + json.dumps(data['data']) + '<br>' + '本次返回数据' + json.dumps(html) + '<br>' + '期望结果为' + data[
        'excepted_code']
    return severity, module, title, pri, steps
